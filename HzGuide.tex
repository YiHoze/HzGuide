\documentclass[minted, pairquote]{hzguide}

\setmainfont{NotoSerifCJKkr-Regular.otf}[BoldFont={NotoSerifCJKkr-Bold.otf}, ItalicFont={NotoSerif-LightItalic.ttf}]
...
{                
    ...
    \CodeSetup{offset=-1.2\onelineskip}
}
...
\newcommand\powershellread{\coderead[language=powershell]}
\newcommand\pythonread{\coderead[language=python]}
...
나는 테크니컬 라이터로서 내게 필요한 것들을 HzGuide 클래스에 만들어 넣었다.
...
다른 종류의 문서들에 비해 설명서가 더 많은 이미지들을 포함하기 때문에 떠다니는 \verb{figure} 환경이 사용하기에 도리어 부적합하다.
...
이 문서는 루아텍으로 컴파일되었다.
지텍으로 컴파일하면 소스 코드를 보여주는 부분에서 결과가 달라질 것이다.
나는 루아텍을 선호한다. 
...
\begin{code}
\documentclass[minted, minted, minted,, minted, mintedlanguage=korean, styleset=../foo.tex, Noto]{hzguide}
\end{code}
...
\macro{language} 클래스 옵션에 주어진 언어의 첫 세 글자가 저장되고, 그것을 \macro{\IfLang} 명령과 \macro{IfLanguage} 환경에 사용할 수 있다.

\begin{code}
\documentclass[minted, minted, minted,, minted, mintedlanguage=german]{hzguide}
\IfLang{ger}{…}
\begin{IfLanguage}{ger} … \end{IfLanguage}
\end{code}
...
\item[paper] \keyvalue{A3, A4, A5, A5V, letter, B5, 46M, slide, arbitrary}
종이 크기들은 다음과 같다.
\begin{macros}*
\item[A3] 297 × 420 mm 
\item[A4] 210 × 297 mm 
...
\item[B5] 176 × 250 mm
\item[46M] 182 × 257 mm \\ 이것은 JIS의 B5 크기이며 흔히 \term{사륙배판}이라고 불린다.
...
\macro{\HeadingSetup} 명령을 이용하여 장과 절 표제의 모양을 그리고 문단 모양을 변경할 수 있다. 
...
이것은 메므와의 \macro{aritcle} 클래스 옵션과 무관하다.

\item[cftsection] \keyvalueTF
차례 표제가 절 수준으로 바뀐다. 

\item[chapterwider] \keyvalueTF
변이단 조판에서 장 표제가 왼쪽 여백에서 시작되게 한다.
...
장 표제 아래에 장 차례가 만들어진다.

\item[sectionstyle] \keyvalue{firm, sparse, tight, multiline, rule}
절 스타일. \macro{multiline}은 변이단 조판에서 절 표제의 둘째 줄이 들여쓰기 되는 것을 방지한다.
...
\begin{code}
\begin{callout}*[옵션] \item \end{callout}
\CalloutSetup{옵션}
\end{code}
...
일반 출판물과 달리 설명서에서 표는 보충적인 자료가 아니라 핵심적인 정보의 일부이기 때문에 절 표제 아래에 오는 표 하나가, 다른 텍스트 없이도, 그 절의 온전한 내용이 될 수 있다. 따라서 불가피한 경우가 아니라면 표에 제목을 붙이지 않는 것이 바람직하다.
...
별표가 붙은 \macro{\begin{options}*}는 항목 사이 간격을 좁힌다.
...
\item[font] \keyvalue{\smfall\sffamily\bfseries}
...
페이지 번호와 절 표제를 포함하는 상호 참조들을 서로 다른 색으로 표시하는 것이 합리적이라고 할 수 없다.
...
각 파일에 고유한 이름을 붙인다면 \term{localtexmf}나 \term{texmfhome} 디렉토리에 파일들을 둘 수 있다.
...
\chapter{텍 라이브 설정하기}
\label:{chp:tlconf_cmd}

HzGuide 클래스와 함께 제공되는 \cmd{tlconf.cmd} 스크립트가 텍 라이브를 설치한 다음에 폰트 경로를 비롯한 설정에 들여야 하는 성가신 수고를 줄여준다.
...
\item[cnf] \macro{texmf.cnf}에, 그리고 \macro{local.cnf}를 생성하여, 사용자 폰트 폴더의 경로를 추가한다.
...
\begin{codewrite}
C:\>jupyter nbconvert --to latex --template FOO.tpl foo.ipynb
\end{codewrite}
\powershellread
