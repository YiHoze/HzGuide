...
\PageStyleSetup{
    headrule=false,
    headouter={},
    evenfootleft={\thepage\ |\quad \leftmark},
    oddfootright={\rightmark\quad |\ \thepage},
    sectionmark=true
}
...
\term{HzGuide} 클래스는 \term{memoir} 클래스에 기반하여 설명서 작성을 돕기 위한 목적으로 만들어졌다. 
...
\item[chapterstyle] \keyvalue{tight}
...
\begin{code}
\ImageSetup{옵션}
\image*|[옵션]{이미지 파일}[다른 이미지 파일](그림 제목)<짧은 제목>
\end{code}
...
그림 차례에 들어가는 짧은 그림 제목

\item[caption-verb] \keyvalueTF
기호 문자들을 백슬래시 없이 그대로\annotate{verbatim} 그림 제목에 사용할 수 있다.

\item[caption-ignore] \keyvalueTF
그림 제목이 식자되지 않는다.

\item[caption-fit] \keyvalueTF
그림 제목의 폭이 이미지 폭에 맞춰진다.
...
\begin{code}
\IllustImageSetup{옵션}
\illustimage*^[옵션]{이미지 파일}(그림 제목){텍스트}
\begin{IllustImage}*^[옵션]{이미지 파일}(그림 제목)
설명문
\end{IllustImage}
\end{code}
...
\macro{\NewBoundedBox} 명령이 이 박스들을 활용하는 환경들을 만들어낼 수 있다.
...
박스 아래 간격. \macro{\hznulldim}이면 \macro{\ObjectSetup}으로 설정된 값이 사용된다.
