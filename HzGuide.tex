...
\CodeSetup{
    ...
    afterskip=-.1\onelineskip,
    offset=-.75\onelineskip %verbatiminput: offset=-1.2\onelineskip
}
\BoundedBoxSetup{
...
\NewDocumentCommand \hzsource { s O{hzguide.tex} m m }
{
...
\makeatletter
\cs_new:Npn \hzsrc_part:nnn #1 #2 #3
{
...
\macro{\IfVartwo} 명령은 클래스 내부에서 여러 명령들의 작동을 제어하기 위해 고안되었다.
...
텍스트 콜아웃이 번호 콜아웃보다 독자에게 더 친절한 것이지만, 삽화 위에 텍스트를 수정하거나 번역하는 일은 불편하다.
...
\begin{code}
\NewConditionals{명령}{환경}{설정}
\end{code}

아래와 같은 선언에 의해 다음과 같은 명령과 환경이 생성되어 있다.

\begin{code}
\NewConditionals{doc}{Doc}{Document}

\ifdoc*^{조건}[추가 조건]{ TRUE }[ FALSE ]
\begin*^{IfDoc}{조건}[추가 조건] TRUE \end{IfDoc}
\DocumentSetup{조건, 조건, ...}
\end{code}
...
이 문서를 작성하기 위하여 레이텍 코드를 처리하는 몇 가지 명령들과 환경들이 만들어졌다.
...
\macro{codewrite} 환경은 소스 코드를 저장하고, \macro{\coderead} 명령은 저장된 소스 코드를 읽어온다.
