...
\item[-v] 출력 파일의 이름이 지정되지 않으면 ---\verb{-o} 옵션이 주어지지 않으면--- 문자열 치환이 그 결과를 원본 파일에 덮어 쓴다. 이 옵션이 주어지면 같은 이름의 파일이 존재할 때 출력을 위한 파일 이름에 번호가 추가된다. 다른 기능이 수행될 때에도 마찬가지이다. 
