...
텍스트 에디터에서 단축키를 이용하여 텍 파일을 컴파일할 수 있지만, \macro{-shell-escape} 옵션을 주거나 xindex 또는 bibtex을 돌려야 하는 것과 같은 다양한 경우들의 각각에 매우 제한적으로 서로 다른 단축키를 할당할 수 있을 뿐이다.
...
예를 들어, \macro{PATH} 시스템 변수에 \macro{C:\texlive\2022\bin\win64}를 설정하고, \macro{alternative_path} 옵션에 \macro{C:\texlive\2022\bin\win32}를 설정하라.
\item[-b] 이것은 \macro{-interaction=batchmode}를 의미한다. 이 옵션이 주어지지 않으면 \macro{-synctex=1}이 사용된다.
\item[-s] 이것은 \macro{-shell-escape}와 같다.
...
\item[-i] 색인이 정렬된다. 
\item[-I] 색인 정렬을 위한 언어를, \macro{german} 또는 \macro{ger}와 같이, 지정하라. 디폴트는 \macro{korean}이다. .ist 또는 .xdy를 지정하는 것도 가능하다.
\item[-a] \macro{-f} 옵션이 주어지면 컴파일 뒤에 .aux를 비롯한 모든 부수 파일들이 삭제된다. 그 파일들을 남겨두려면 이 옵션을 사용하라.
...
\item[-I] EPS를 PDF로 또는 그 반대로 변환하는 데에, \macro{epstopdf.exe}와 \macro{pdftops} 대신, 잉크스케이프가 사용된다. 
...
\item[-o] \macro{-a} 옵션과 함께, 응용 프로그램에 전달할 옵션들을 지정하라.
...
\item[-g] 문자열들을 추출하고 한 파일에 합쳐 저장한다. 다음 예가 모든 텍 파일에서 단어들을 추출한다. 달리 지정하지 않으면 출력 파일의 이름이 \verb{extracted_strings.txt}이다.
...
\item[-l] 이 옵션이 주어지지 않으면, 출력 파일들이 op.py에 의해 열린다.
\end{macros}
...
목적 디렉토리를 지정하지 않으면, \macro{_bak} 디렉토리가 만들어지고, 그 아래에 파일들이 복사된다.
\macro{-a}나 \macro{--affix} 옵션을 사용하여 파일들의 이름에 붙일 문자열을 지정할 수 있다.
...
\macro{-f} 또는 \macro{--flag} 옵션을 사용하여 백업 방식을 지정할 수 있다.
...
다른 파일들을 함께 저장하려면 위 예와 같이 접미어 \macro{_output}이 포함된 이름과 함께 파일을 지정하라.
...
다른 파일들을 함께 저장하려면 위 예와 같이 `\macro{_output}'이 포함된 이름과 함께 파일을 지정하라.
그러면 데이터베이스에 다음과 같이 추가될 것이다.
...
\item[-N] 지정된 스크립트가 추출된 다음에 실행되지 않는다.
